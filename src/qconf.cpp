...
public:
    ConfUsageOpt(const QString &_name = "", const QString &_arg = "", const QString &_desc = "") :
        name(_name), arg(_arg), desc(_desc)
    {
    }
...
public:
    ConfOpt(const QString &_name = "", const QString &_arg = "", const QString &_var = "", const QString &_desc = "") :
        name(_name), arg(_arg), var(_var), desc(_desc)
    {
    }
...
private:
    QString genHeader()
    {
        QString str = QString("#!/bin/sh\n"
                              "#\n"
                              "# Generated by qconf %1 ( https://github.com/psi-plus/qconf )\n"
                              "#\n"
                              "\n")
                          .arg(VERSION);
        return str;
    ...
    QString genFindStuff()
    {
        ...
        // which
        str += "# which/make detection adapted from Qt\n"
               "which_command() {\n"
               "	ALL_MATCHES=\n"
               "	if [ \"$1\" = \"-a\" ]; then\n"
               "		ALL_MATCHES=\"-a\"\n"
    ...
    {
        QString str = "if [ -z \"$QC_QTSELECT\" ]; then\n"
        ...
        str += "if [ \"$QC_VERBOSE\" = \"Y\" ]; then\n"
               ...
               "				cmd=\"$qtchooser -run-tool=qmake -qt=${version} -query "
               "QT_INSTALL_BINS\"\n"
               "				if [ \"$QC_VERBOSE\" = \"Y\" ]; then\n"
               "					echo \"running: $cmd\"\n"
               "				fi\n"
    ...
    for (QList<Dep>::Iterator it = conf.deps.begin(); it != conf.deps.end(); ++it) {
        ...
        if (dep.pkgconfig) {
            ...
            modscreate += QString("    o = new qc_internal_pkgconfig(conf, \"%1\", \"%2\", %3, \"%4\");\n    "
                              ...
                              .arg(modestr)
                              .arg(dep.pkgver)
                              .arg(dep.required ? "true" : "false")
                              .arg(dep.disabled ? "true" : "false");
            continue;
        }
